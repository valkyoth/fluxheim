...
| `crates/fluxheim-server/src/native_http1_tls.rs` | 773 | Native upstream TLS connector covers rustls/OpenSSL, trust roots, SNI, hostname policy, and mTLS material after file loading moved to a focused child module. | Split rustls, OpenSSL, and hostname-policy helpers before production native proxy cutover. |
