...
| `src/metrics.rs` | 626 | Root metrics recording/export adapter over many domains after native metrics HTTP app, secret loading, label mapping, registry storage, constructor helpers, and metrics tests moved to focused private modules. | Move remaining pure metrics into `fluxheim-observability`. |
