...
| `crates/fluxheim-server/src/native_http1_proxy_tests.rs` | 1327 | Native HTTP/1 proxy parity tests temporarily group plain, TLS, mTLS, pooling, compression, and error-page fixtures for the native upstream cutover slices; HTTP/2 parity and fixtures, unsupported-policy config, static upstream failover/round-robin, and header/forwarded-policy tests now live in focused child modules. | Split TLS/mTLS, pooling, compression/error-page, and remaining config tests after the native proxy API stabilizes. |
