...
| `crates/fluxheim-server/src/native_http1_route_proxy_tests.rs` | 4228 | Native route-proxy tests temporarily group request/response header overlays, forwarded-header ownership, response rewrites, compression, cache, PHP, and proxy fixtures for the rich route cutover slices after redirect hardening, cache/config admission, and routing/policy tests moved to focused test modules. | Split header-policy, forwarded-header, response-rewrite, compression, cache runtime, PHP, and remaining proxy route tests before final native runtime cleanup completes. |
