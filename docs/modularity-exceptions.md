...
| `src/native_proxy.rs` | 559 | Native runtime admin/cache boundary used by root admin, CLI, metrics, and reload code after the Pingora adapter stopped compiling in normal builds; load-balancer admin handlers, cache runtime stats, and cache purge handlers now live in focused child modules. | Split snapshot/cache-preview and live-config reload glue into focused native modules during the `1.6.37` structural cleanup. |
