...
| `crates/fluxheim-server/src/native_http1_route_proxy_tests.rs` | 1587 | Native route-proxy tests temporarily group cache behavior, rewrite, ACME, WebSocket, and proxy fixtures for the rich route cutover slices after redirect hardening, cache/config admission, routing/policy, header/forwarded-header, compression/response-header, access/rate/concurrency, PHP-FPM, cache-storage, cache range/slice, peer-fill, and cache freshness tests moved to focused test modules. | Split remaining cache behavior, ACME, WebSocket, and proxy route tests before final native runtime cleanup completes. |
