...
| `crates/fluxheim-server/src/native_http1_plan_tests.rs` | 1129 | Native HTTP/1 cutover planning tests grew during the native route/policy parity slices to cover compression, access, rate-limit, concurrency, redirect, load-balancer, and service-readiness candidates together; root/protocol, cache/static-web/PHP, and route header-policy candidates now live in focused child modules. | Split access/rate/concurrency candidates, load-balancer candidates, compression eligibility tests, and summary/state candidates before final native runtime cleanup completes. |
