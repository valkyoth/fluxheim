...
| `crates/fluxheim-server/src/native_http1_cache.rs` | 1449 | Native cache entry/state, filesystem disk cache, and storage-bin disk cache remain grouped after cache tests, disk metadata, safe filesystem paths, encryption/OpenBao helpers, and purge/inspection APIs moved to child modules. | Split memory state, filesystem disk storage, and storage-bin storage before final native runtime cutover. |
