...
| `crates/fluxheim-server/src/native_http1_route_proxy.rs` | 582 | Native route proxy temporarily groups route matching, safe redirect expansion, request-body limits, and response-header overlays during the `1.6.26` route/policy parity slice. | Split redirect handling and response-header policy helpers into focused modules as the remaining route policies land. |
