...
| `crates/fluxheim-config/src/config_proxy.rs` | 1295 | Proxy config and validation after auth-request, traffic-mirror, error-page config, shared protocol enums, and upstream subset policy helpers moved out. | Split upstream discovery and TLS/upstream policy subdomains. |
