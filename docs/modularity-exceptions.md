...
| `crates/fluxheim-server/src/native_http1_plan_tests.rs` | 1569 | Native HTTP/1 cutover planning tests grew during the native route/policy parity slices to cover header mutation, compression, access, rate-limit, concurrency, redirect, load-balancer, and service-readiness candidates together; root/protocol and cache/static-web/PHP candidate tests now live in focused child modules. | Split header-policy candidates, access/rate/concurrency candidates, load-balancer candidates, compression eligibility tests, and summary/state candidates before final native runtime cleanup completes. |
