...
| `crates/fluxheim-server/src/native_http1_cache.rs` | 755 | Shared native disk-cache orchestration remains grouped after cache tests, memory state/helpers, disk backend/state definitions, filesystem storage, storage-bin operations, disk metadata, safe filesystem paths, encryption/OpenBao helpers, and purge/inspection APIs moved to child modules. | Split shared get/store/purge orchestration into smaller method-group modules before final native runtime cutover. |
