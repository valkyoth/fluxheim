...
| `crates/fluxheim-server/src/native_http1_plan_tests.rs` | 2035 | Native HTTP/1 cutover planning tests grew during the native route/policy parity slices to cover cache, PHP, static-web, header mutation, compression, access, rate-limit, concurrency, redirect, load-balancer, and service-readiness candidates together; root/protocol candidate tests now live in a focused child module. | Split cache/PHP/static-web candidates, header-policy candidates, access/rate/concurrency candidates, load-balancer candidates, and compression eligibility tests before final native runtime cleanup completes. |
