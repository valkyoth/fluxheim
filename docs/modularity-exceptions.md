...
| --- | ---: | --- | --- |
| `src/acme.rs` | 1763 | ACME order/install/renewal and filesystem safety in one root adapter after regression coverage, error typing, challenge stores, renewal queue helpers, PEM validation, EAB secret handling, account credential storage, and certificate path/observation helpers moved to focused child modules. | Move to `fluxheim-acme` after the native listener/TLS cutover stabilizes. |
