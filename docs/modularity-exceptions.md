...
| `crates/fluxheim-server/src/native_http1_route_proxy_tests.rs` | 643 | Native route-proxy tests temporarily retain shared network fixture servers and listener/client helpers for the rich route cutover slices after redirect hardening, cache/config admission, routing/policy, header/forwarded-header, compression/response-header, access/rate/concurrency, PHP-FPM, cache behavior, rewrite, ACME, WebSocket, header-default/privacy-mode, and support-builder tests moved to focused modules. | Split remaining network fixture helpers where practical before final native runtime cleanup completes. |
