...
| --- | ---: | --- | --- |
| `crates/fluxheim-config/src/config_tests.rs` | 2119 | Legacy central config regression suite after basic server/process/static-web, server listener/limit, vhost/route, compression, GeoIP, generic header validation, header policy, proxy timeout, proxy upstream, load-balancer, cache, PHP-FPM, TLS/ACME/FIPS, admin, observability, and logging coverage moved to focused child modules. | Split by config domain as crates stabilize. |
