...
| --- | ---: | --- | --- |
| `src/cli.rs` | 2502 | Legacy command dispatch and release/admin/cache tooling after CLI regression coverage, crypto/cache-keygen command helpers, ACME CLI helpers, and cache-warm command helpers moved to focused child modules. | Split command handlers by domain after runtime crates settle. |
