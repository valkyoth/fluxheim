...
| `crates/fluxheim-server/src/native_http1_proxy_memory_cache.rs` | 675 | Native proxy memory-cache lookup, peer-fill, stale/revalidation, and predictor runtime were moved together so cache behavior stays reviewable while the proxy file is split; slice-fill, store/admission, and disk promotion now live in focused child modules. | Split lookup, peer-fill, and predictor helpers after the proxy orchestration file stabilizes. |
