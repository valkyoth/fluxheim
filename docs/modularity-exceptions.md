...
| `crates/fluxheim-server/src/native_http1_plan_tests.rs` | 837 | Native HTTP/1 cutover planning tests grew during the `1.6.26`-`1.6.27` route/policy parity slices to cover request-header mutations, response-header overlays, response rewrites, and redirect-shadowed route proxies together. | Split route-policy candidate tests from vhost/root candidate tests before the final Pingora dependency deletion. |
| `crates/fluxheim-server/src/native_http1_route_proxy.rs` | 683 | Native route proxy temporarily groups route matching, safe redirect expansion, request-body limits, request/response-header overlays, and static-web route action dispatch during the `1.6.26`-`1.6.27` route/rich-integration parity slices. | Split redirect handling, header policy helpers, and static-web dispatch into focused modules before final native runtime cutover. |
