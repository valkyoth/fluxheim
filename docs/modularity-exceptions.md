...
| `crates/fluxheim-server/src/native_http1_proxy_memory_cache.rs` | 788 | Native proxy memory-cache lookup, disk promotion, peer-fill, stale/revalidation, and predictor runtime were moved together so cache behavior stays reviewable while the proxy file is split; slice-fill and store/admission now live in focused child modules. | Split lookup, peer-fill, disk promotion, and predictor helpers after the proxy orchestration file stabilizes. |
