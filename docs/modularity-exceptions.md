...
| `crates/fluxheim-config/src/config_load_balance.rs` | 789 | Load-balancer selection, persistence config, and shared validation after health-check, passive-health, retry, and queue config moved out. | Split remaining persistence subdomain and selection helpers. |
