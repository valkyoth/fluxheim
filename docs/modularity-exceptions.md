...
| `crates/fluxheim-server/src/native_http1_proxy_memory_cache.rs` | 594 | Native proxy memory-cache lookup, stale/revalidation, and predictor runtime were moved together so cache behavior stays reviewable while the proxy file is split; slice-fill, store/admission, disk promotion, and peer-fill now live in focused child modules. | Split lookup and predictor helpers after the proxy orchestration file stabilizes. |
