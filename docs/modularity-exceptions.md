...
| `crates/fluxheim-server/src/native_http1_route_proxy_tests.rs` | 997 | Native route-proxy tests temporarily group WebSocket, header-default/privacy-mode, and proxy fixtures for the rich route cutover slices after redirect hardening, cache/config admission, routing/policy, header/forwarded-header, compression/response-header, access/rate/concurrency, PHP-FPM, cache behavior, rewrite, and ACME tests moved to focused test modules. | Split WebSocket, header-default/privacy-mode, and remaining proxy route tests before final native runtime cleanup completes. |
