...
| --- | ---: | --- | --- |
| `crates/fluxheim-config/src/config_tests.rs` | 4359 | Legacy central config regression suite after basic server/process/static-web, compression, GeoIP, generic header validation, proxy timeout, proxy upstream, load-balancer, cache, PHP-FPM, TLS/ACME/FIPS, admin, observability, and logging coverage moved to focused child modules. | Split by config domain as crates stabilize. |
