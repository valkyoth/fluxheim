...
| --- | ---: | --- | --- |
| `crates/fluxheim-config/src/config_tests.rs` | 10160 | Legacy central config regression suite after basic server/process/static-web, compression, GeoIP, generic header validation, proxy timeout, load-balancer, and cache coverage moved to focused child modules. | Split by config domain as crates stabilize. |
