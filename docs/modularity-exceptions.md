...
| `src/acme.rs` | 2392 | ACME account/order/install/renewal and filesystem safety in one root adapter after regression coverage, error typing, challenge stores, and renewal queue helpers moved to focused child modules. | Move to `fluxheim-acme` after the native listener/TLS cutover stabilizes. |
