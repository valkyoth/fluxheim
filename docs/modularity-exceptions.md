...
| `src/tls.rs` | 544 | Root TLS storage, ACME path resolution, and compatibility glue after TLS unit tests and storage issue formatting moved to focused modules and `fluxheim-tls` extracted listener planning, provider, ALPN, cipher, and SNI selector policy. | Move remaining ACME/storage helpers to focused crates after native listener cutover. |
