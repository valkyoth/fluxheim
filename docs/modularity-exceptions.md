...
| `src/acme.rs` | 3004 | ACME account/order/install/renewal and filesystem safety in one root adapter after regression coverage moved to focused child test modules. | Move to `fluxheim-acme` after the native listener/TLS cutover stabilizes. |
