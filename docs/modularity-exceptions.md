...
| --- | ---: | --- | --- |
| `src/cli.rs` | 946 | Legacy command dispatch and release/admin/cache tooling after CLI regression coverage, crypto/cache-keygen command helpers, ACME CLI helpers, cache-warm command helpers, cache-key preview helpers, cache object lookup helpers, runtime validation, and TLS storage checks moved to focused child modules. | Split remaining command dispatch/options by domain after runtime crates settle. |
