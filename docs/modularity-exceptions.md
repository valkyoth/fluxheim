...
| `src/metrics.rs` | 3212 | Root metrics registry/export adapter over many domains after native metrics HTTP app and secret loading moved to focused private modules. | Move remaining pure metrics into `fluxheim-observability`. |
...
| `src/runtime.rs` | 1165 | Native server/bootstrap/listener orchestration after runtime unit tests moved to a focused test module; TLS listener planning and server process/listener inventory now come from focused crates while this file remains the compatibility adapter. | Continue replacing through `fluxheim-runtime`/`fluxheim-server` during the remaining `1.6` runtime cutover. |
| `src/tls.rs` | 782 | Root TLS storage, ACME path resolution, and compatibility glue after TLS unit tests moved to focused modules and `fluxheim-tls` extracted listener planning, provider, ALPN, cipher, and SNI selector policy. | Move remaining ACME/storage helpers to focused crates after native listener cutover. |
