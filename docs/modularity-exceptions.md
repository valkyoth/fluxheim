...
| --- | ---: | --- | --- |
| `src/admin.rs` | 2571 | Legacy admin HTTP endpoint router over every domain after admin endpoint, auth, cache, load-balancer, snapshot, self-healing, shared regression-test support, cache response helper blocks, admin security helpers, token-store helpers, response builders, cache purge validation, and misc status/query helpers moved to child modules. | Continue splitting admin domain helpers, then evaluate `fluxheim-admin`. |
