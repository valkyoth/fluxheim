...
| --- | ---: | --- | --- |
| `src/admin.rs` | 1397 | Legacy admin HTTP endpoint router over every domain after admin endpoint, auth, cache, load-balancer, UDP, snapshot, self-healing, shared regression-test support, cache response helper blocks, admin security helpers, token-store helpers, response builders, cache purge validation, misc status/query helpers, load-balancer/UDP endpoint handlers, and request-router dispatch moved to child modules. | Continue splitting admin domain helpers, then evaluate `fluxheim-admin`. |
