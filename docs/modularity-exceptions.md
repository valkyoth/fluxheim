...
| `crates/fluxheim-server/src/native_http1_route_proxy_tests.rs` | 810 | Native route-proxy tests temporarily retain shared proxy fixture servers, request builders, and config builders for the rich route cutover slices after redirect hardening, cache/config admission, routing/policy, header/forwarded-header, compression/response-header, access/rate/concurrency, PHP-FPM, cache behavior, rewrite, ACME, WebSocket, and header-default/privacy-mode tests moved to focused test modules. | Split remaining shared fixture builders where practical before final native runtime cleanup completes. |
