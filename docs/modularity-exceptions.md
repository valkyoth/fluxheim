...
| `crates/fluxheim-server/src/native_http1_client.rs` | 1575 | Native upstream HTTP/1 client temporarily groups connection pooling, request writing, response parsing, stale-connection retry, timeout handling, and forwarded-header-safe request construction after socket setup, HTTP/2 conversion, and upgrade validation helpers moved into focused child modules. | Split pooling, request writer, retry/timeout policy, and request-construction helpers before final native runtime cutover. |
