...
| `crates/fluxheim-config/src/config_load_balance.rs` | 1165 | Load-balancer selection, passive health, retry, persistence, queue config, and shared validation after health-check schema/validation moved out. | Split remaining passive-health, retry, persistence, and queue subdomains. |
