...
| --- | ---: | --- | --- |
| `src/cli.rs` | 1178 | Legacy command dispatch and release/admin/cache tooling after CLI regression coverage, crypto/cache-keygen command helpers, ACME CLI helpers, cache-warm command helpers, cache-key preview helpers, and cache object lookup helpers moved to focused child modules. | Split remaining command handlers by domain after runtime crates settle. |
