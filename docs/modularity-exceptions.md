...
| `crates/fluxheim-config/src/config_load_balance.rs` | 871 | Load-balancer selection, persistence, queue config, and shared validation after health-check, passive-health, and retry config moved out. | Split remaining persistence and queue subdomains. |
