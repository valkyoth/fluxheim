...
| `crates/fluxheim-server/src/native_http1_cache.rs` | 939 | Native filesystem disk cache and shared disk cache flow remain grouped after cache tests, memory state/helpers, disk backend/state definitions, storage-bin operations, disk metadata, safe filesystem paths, encryption/OpenBao helpers, and purge/inspection APIs moved to child modules. | Split filesystem disk storage before final native runtime cutover. |
