...
| `crates/fluxheim-server/src/native_http1.rs` | 934 | Native downstream HTTP/1 listener, TLS accept loop, connection budget, and request-head parsing remain grouped after response framing, request metadata, PROXY protocol parsing, and body reading moved to focused child modules. | Split plain listener, TLS accept loops, connection serving, and request-head parsing before the final 1.6 cleanup completes. |
