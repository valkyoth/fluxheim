...
| `crates/fluxheim-server/src/native_http1_client.rs` | 1298 | Native upstream HTTP/1 client temporarily groups connection orchestration, response parsing, and timeout handling after socket setup, request writing/header filtering, HTTP/2 conversion, upgrade validation, pool state, and retry predicates moved into focused child modules. | Split connection orchestration, timeout policy, and remaining response-handling helpers before final native runtime cutover. |
