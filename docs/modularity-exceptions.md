...
| `crates/fluxheim-server/src/native_http1_plan_tests.rs` | 704 | Native HTTP/1 cutover planning tests grew during the `1.6.26` route/policy parity slice to cover response-header overlays, response rewrites, and redirect-shadowed route proxies together. | Split route-policy candidate tests from vhost/root candidate tests before the final Pingora dependency deletion. |
| `crates/fluxheim-server/src/native_http1_route_proxy.rs` | 621 | Native route proxy temporarily groups route matching, safe redirect expansion, request-body limits, response-header overlays, and static-web route action dispatch during the `1.6.26`-`1.6.27` route/rich-integration parity slices. | Split redirect handling, response-header policy helpers, and static-web dispatch into focused modules before final native runtime cutover. |
