...
| `crates/fluxheim-server/src/native_http1_proxy_tests.rs` | 1497 | Native HTTP/1 proxy parity tests temporarily group plain, TLS, mTLS, pooling, compression, error-page, and forwarded-header fixtures for the native upstream cutover slices; HTTP/2 parity and fixtures, unsupported-policy config, and static upstream failover/round-robin tests now live in focused child modules. | Split TLS/mTLS, pooling, compression/error-page, forwarded-header, and remaining config tests after the native proxy API stabilizes. |
