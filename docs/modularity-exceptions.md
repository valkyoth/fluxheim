...
| `crates/fluxheim-server/src/native_http1_route_proxy_tests.rs` | 916 | Native route-proxy tests temporarily group header-default/privacy-mode checks and proxy fixtures for the rich route cutover slices after redirect hardening, cache/config admission, routing/policy, header/forwarded-header, compression/response-header, access/rate/concurrency, PHP-FPM, cache behavior, rewrite, ACME, and WebSocket tests moved to focused test modules. | Split header-default/privacy-mode checks and remaining proxy route fixtures before final native runtime cleanup completes. |
