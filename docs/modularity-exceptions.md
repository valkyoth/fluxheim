...
| --- | ---: | --- | --- |
| `src/admin.rs` | 4475 | Legacy admin HTTP endpoint router over every domain after admin endpoint, auth, cache, load-balancer, snapshot, and self-healing regression coverage moved to child modules. | Continue splitting admin domain helpers, then evaluate `fluxheim-admin`. |
