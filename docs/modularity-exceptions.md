...
| `crates/fluxheim-config/src/config_load_balance.rs` | 1033 | Load-balancer selection, retry, persistence, queue config, and shared validation after health-check and passive-health config moved out. | Split remaining retry, persistence, and queue subdomains. |
