...
| --- | ---: | --- | --- |
| `crates/fluxheim-config/src/config_tests.rs` | 8025 | Legacy central config regression suite after basic server/process/static-web, compression, GeoIP, generic header validation, proxy timeout, load-balancer, cache, and PHP-FPM coverage moved to focused child modules. | Split by config domain as crates stabilize. |
