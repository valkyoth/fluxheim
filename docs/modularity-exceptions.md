...
| `crates/fluxheim-server/src/native_http1_cache.rs` | 1779 | Native cache entry/state, filesystem disk cache, storage-bin disk cache, inspection, and purge helpers remain grouped after cache tests, disk metadata, safe filesystem paths, and encryption/OpenBao helpers moved to child modules. | Split memory state, filesystem disk storage, storage-bin storage, inspection, and purge/index helpers before final native runtime cutover. |
