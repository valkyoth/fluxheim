...
| `src/native_proxy.rs` | 1264 | Native runtime admin/cache boundary used by root admin, CLI, metrics, and reload code after the Pingora adapter stopped compiling in normal builds; load-balancer admin handlers now live in a focused child module. | Split cache runtime, purge, snapshot/cache-preview, and live-config reload glue into focused native modules during the `1.6.37` structural cleanup. |
