...
| --- | ---: | --- | --- |
| `src/cli.rs` | 1948 | Legacy command dispatch and release/admin/cache tooling after CLI regression coverage, crypto/cache-keygen command helpers, ACME CLI helpers, cache-warm command helpers, and cache-key preview helpers moved to focused child modules. | Split command handlers by domain after runtime crates settle. |
