...
| `crates/fluxheim-server/src/native_http1_client.rs` | 1004 | Native upstream HTTP/1 client temporarily groups HTTP/1 connection orchestration, response parsing, and timeout handling after socket setup, request writing/header filtering, HTTP/2 conversion/send orchestration, upgrade validation, pool state, and retry predicates moved into focused child modules. | Split HTTP/1 connection orchestration, timeout policy, and remaining response-handling helpers before final native runtime cutover. |
