...
| `crates/fluxheim-server/src/native_http1_cache.rs` | 2390 | Native cache entry/state, filesystem disk cache, storage-bin disk cache, local/OpenBao encryption, inspection, and purge helpers remain grouped after cache tests, disk metadata, and safe filesystem path helpers moved to child modules. | Split memory state, filesystem disk storage, storage-bin storage, encryption, inspection, and purge/index helpers before final native runtime cutover. |
