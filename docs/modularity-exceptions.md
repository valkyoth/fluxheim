...
| --- | ---: | --- | --- |
| `src/acme.rs` | 866 | ACME order and renewal orchestration still live in one root adapter after regression coverage, error typing, challenge stores, renewal queue helpers, PEM validation, EAB secret handling, account credential storage, certificate path/observation helpers, HTTP-01/TLS-ALPN helpers, client error adaptation, naming/token helpers, and certificate install/backup/restore helpers moved to focused child modules. | Move to `fluxheim-acme` after the native listener/TLS cutover stabilizes. |
