...
| `crates/fluxheim-load-balancer/src/health.rs` | 2484 | Multiple active health-check protocols in one file after the 1.6.17 native HTTP/1 and h2/gRPC probes replaced Pingora health sessions. | Split by protocol during the remaining load-balancer/runtime cutover: TCP/TLS, HTTP/1, gRPC/h2, Redis, MySQL, PostgreSQL, exec, and shared validators. |
