...
| `crates/fluxheim-server/src/native_http1_proxy_memory_cache.rs` | 1082 | Native proxy memory-cache lookup, disk promotion, peer-fill, stale/revalidation, store, and predictor runtime were moved together so cache behavior stays reviewable while the proxy file is split; slice-fill now lives in a focused child module. | Split lookup, peer-fill, disk promotion, and store/admission helpers after the proxy orchestration file stabilizes. |
