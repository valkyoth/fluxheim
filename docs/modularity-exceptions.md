...
| `crates/fluxheim-server/src/native_http1_client.rs` | 1381 | Native upstream HTTP/1 client temporarily groups connection pooling, response parsing, stale-connection retry, and timeout handling after socket setup, request writing/header filtering, HTTP/2 conversion, and upgrade validation helpers moved into focused child modules. | Split pooling, retry/timeout policy, and remaining response-handling helpers before final native runtime cutover. |
