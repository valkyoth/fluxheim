...
| `crates/fluxheim-config/src/config_load_balance.rs` | 515 | Load-balancer selection, slow-start config, and shared validation after health-check, passive-health, retry, queue, and persistence config moved out. | Split remaining slow-start and selection helpers. |
