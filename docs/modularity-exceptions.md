...
| `crates/fluxheim-server/src/native_http1_client.rs` | 1698 | Native upstream HTTP/1 client temporarily groups connection pooling, request writing, response parsing, stale-connection retry, timeout handling, and forwarded-header-safe request construction after socket setup and HTTP/2 conversion helpers moved into focused child modules. | Split pooling, request writer, upgrade handling, retry/timeout policy, and request-construction helpers before final native runtime cutover. |
