...
| `crates/fluxheim-server/src/native_http1_plan_tests.rs` | 828 | Native HTTP/1 cutover planning tests grew during the native route/policy parity slices to cover compression, redirect, load-balancer, and service-readiness candidates together; root/protocol, cache/static-web/PHP, route header-policy, and access/rate/concurrency candidates now live in focused child modules. | Split load-balancer candidates, compression eligibility tests, and summary/state candidates before final native runtime cleanup completes. |
