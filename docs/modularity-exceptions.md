...
| `crates/fluxheim-server/src/native_http1.rs` | 1037 | Native downstream HTTP/1 listener, TLS accept loop, connection budget, and request parsing remain grouped after response framing, request metadata, and PROXY protocol parsing moved to focused child modules. | Split plain listener, TLS accept loops, connection serving, and request parsing before the final 1.6 cleanup completes. |
