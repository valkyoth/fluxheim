...
| `crates/fluxheim-server/src/native_http1_route_proxy_tests.rs` | 3669 | Native route-proxy tests temporarily group compression, cache, PHP, access/rate/concurrency policy, rewrite, ACME, WebSocket, and proxy fixtures for the rich route cutover slices after redirect hardening, cache/config admission, routing/policy, and header/forwarded-header tests moved to focused test modules. | Split compression, cache runtime, PHP, access/rate/concurrency, ACME, WebSocket, and remaining proxy route tests before final native runtime cleanup completes. |
