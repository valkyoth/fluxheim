...
| --- | ---: | --- | --- |
| `src/admin.rs` | 4639 | Legacy admin HTTP endpoint router over every domain after basic admin/auth, cache, load-balancer, snapshot, and self-healing regression coverage moved to child modules. | Continue splitting admin tests and domain helpers, then evaluate `fluxheim-admin`. |
