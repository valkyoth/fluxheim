...
| `crates/fluxheim-server/src/native_http1_route_proxy_tests.rs` | 823 | Native route-proxy tests temporarily group route matching, safe redirect expansion including encoded and double-encoded path hardening, request/response header overlays, response rewrites, compression, and route static-web fixtures for the rich route cutover slices. | Split redirect, header-policy, response-rewrite, compression, and static-web route tests before final native runtime cutover. |
