...
| `crates/fluxheim-server/src/native_http1_proxy_tests.rs` | 541 | Native HTTP/1 proxy parity tests temporarily group plain, TLS, mTLS, and pooling fixtures for the native upstream cutover slices; HTTP/2 parity and fixtures, unsupported-policy config, static upstream failover/round-robin, header/forwarded-policy, compression, error-page, WebSocket takeover, auth-request, traffic-mirror, and static construction/config tests now live in focused child modules. | Split TLS/mTLS, pooling, and remaining config tests after the native proxy API stabilizes. |
