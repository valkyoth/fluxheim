...
`1.6.19` adds a release-gate dependency proof for
`--no-default-features --features web,tls-rustls`; that build does not compile
Pingora crates, but it is not yet the production static-site runtime profile.
