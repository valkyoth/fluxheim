...
- The `1.2.4` distributed-cache line starts with a safe `[cache.peer_fill]`
  ...
  local post-fill `HIT`, and peer-fill metrics. The configured
  `peer_fill.max_concurrent_requests` budget is enforced per vhost or route
  cache policy for active outbound peer fetches. Peer response `Age` is
  preserved during admission, so a peer-filled object stores only its remaining
  freshness instead of extending the origin TTL. Metrics builds expose aggregate
