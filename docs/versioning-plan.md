...
- Proxy cache HIT `Age`, conditional `304`/`200` validator match and mismatch
  ...
  tested against real cached objects, alongside prefix and wildcard purge.
...
- Cache hits emit correct validator/freshness behavior, including `Age` where
  ...
  misses, disk HIT after restart, admin stale dry-run and prefix/tag/wildcard
  purge against real cached objects, and `Cache-Control`/`Pragma`
