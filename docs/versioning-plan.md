...
- `v1.5.20`: web, PHP-FPM, and cache boundary preparation line. Stop at
  ...
  classifiers for host-routing, admin-auth, compression, edge-policy, stream,
  ACME, and metrics-OTLP exporter events also now live in the observability
  crate while root `crate::metrics` remains the Prometheus registry/recorder
  adapter.
