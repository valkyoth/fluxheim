...
| Load balancing | ✅ | Weighted round-robin, least/weighted/ratio least connections, least-time EWMA, priority groups, per-upstream in-flight caps, power-of-two, hash, consistent-hash, backup, drain, disabled members, runtime drain/disable/enable, slow start, retry budgets/statuses, and configurable all-down status. |
...
| Load-balancer status | ✅ | Admin status includes configured pools, selection/health/retry policy metadata, ready/available summary counts, backend readiness, disabled/drained state, in-flight counts, passive failure/ejection state, slow-start, and least-time latency state. |
