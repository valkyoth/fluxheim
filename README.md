...
| `wasm` | No | Optional `1.7.x` WebAssembly policy runtime. `1.7.7` supports live native HTTP/1 access-decision hooks, bounded request/response header hooks, constrained route-decision hooks with configured canary and mirror branch selection, selected native load-balanced/persistent routes, bounded cache-lookup hooks, bounded cache-store skip/deny hooks, bounded symbolic cache-key component hooks, fixed-ID cache-store TTL/tag/header metadata, compiled-module identities scoped by plugin digest, ABI version, feature surface, host-call namespace, and Fluxheim version, plus an opt-in proxy-ABI preview boundary. Later `1.7.x` releases add direct backend choice, plugin-provided persistence keys, dynamic mirror/shadow target choice, and richer store policy hooks. |
...
`v1.7.7-wolfi`, `v1.7.7-cache-wolfi`, `v1.7.7-proxy-wolfi`,
`v1.7.7-load-balancer-wolfi`, and `v1.7.7-php-wolfi`.
