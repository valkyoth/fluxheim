...
| `wasm` | No | Optional `1.7.x` WebAssembly policy runtime. `1.7.6` supports live native HTTP/1 access-decision hooks, bounded request/response header hooks, constrained route-decision hooks with configured canary and mirror branch selection, selected native load-balanced/persistent routes, bounded cache-lookup hooks, bounded cache-store skip/deny hooks, bounded symbolic cache-key component hooks, fixed-ID cache-store TTL/tag/header metadata, and compiled-module identities scoped by plugin digest, ABI version, feature surface, and Fluxheim version. Later `1.7.x` releases add direct backend choice, plugin-provided persistence keys, dynamic mirror/shadow target choice, and richer store policy hooks. |
...
`v1.7.6-wolfi`, `v1.7.6-cache-wolfi`, `v1.7.6-proxy-wolfi`,
`v1.7.6-load-balancer-wolfi`, and `v1.7.6-php-wolfi`.
