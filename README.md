...
| UDP/GSLB beta boundary | Limited | `1.5.16`; separate `[udp]` config namespace and `udp-proxy` feature gate with beta DNS-style request/response forwarding and syslog one-way forwarding. QUIC pass-through, game proxying, production UDP support, and generic UDP/GSLB platform behavior are not included yet. |
