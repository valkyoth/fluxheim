...
| WASM sandbox foundation | 🧪 | `1.7.0`/`1.7.1`; optional `wasm` feature with strict plugin-file loading, bounded real-Wasm execution smoke, config-level plugin registry validation, typed loader manifests, and authenticated registry status visibility. Policy hooks remain staged for later `1.7.x`. |
