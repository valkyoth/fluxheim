...
| Active health checks | ✅ | TCP/TLS, HTTP, standard gRPC, Redis `PING`, MySQL/MariaDB handshake, exact JSON scalar body validation, `X-Health-Weight` degraded weight signals, and opt-in bounded local exec checks. Agent checks and additional database protocol probes are planned across later `1.5.x` health-check lines. |
