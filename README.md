...
| WASM sandbox foundation | 🧪 | `1.7.0`-`1.7.2`; optional `wasm` feature with strict plugin-file loading, bounded real-Wasm execution smoke, config-level plugin registry validation, typed loader manifests, authenticated registry status visibility, deterministic attachment ordering, global admission limits, and Wasm-aware reload classification. Policy hooks remain staged for later `1.7.x`. |
