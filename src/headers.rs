...
mod tests {
    ...
    fn cookie_domain_rewrite_matches_symmetric_leading_dot_rules() {
        let rewrite = crate::config::ResponseHeaderRewriteConfig {
            cookie_domain: vec![crate::config::ResponseHeaderRewriteRuleConfig {
                from: ".backend.internal".to_owned(),
                to: "example.test".to_owned(),
            }],
        ...
        assert_eq!(
            fluxheim_headers::rewrite_set_cookie_value(
                "session=1; Domain=backend.internal; HttpOnly",
                &rewrite
            ),
            Some("session=1; Domain=example.test; HttpOnly".to_owned())
        );
        assert_eq!(
            fluxheim_headers::rewrite_set_cookie_value(
                "session=1; Domain=.BACKEND.internal; HttpOnly",
