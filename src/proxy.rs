...
pub use crate::cache_api::{
    ...
    CacheKeyPreview, CacheKeyPreviewScope, CacheObjectLookup, CachePurgeRequest, CachePurgeResult,
    CacheRouteStats, CacheRuntimeStats, CacheRuntimeTotals, CacheStalePurgeRequest,
    CacheStalePurgeResult, CacheVhostStats,
};
#[cfg(any(
...
use crate::config::{
    ...
    ServerLimitsConfig, UpstreamHttpVersion, normalize_host,
};
...
use crate::proxy_protocol::apply_upstream_proxy_protocol as install_upstream_proxy_protocol;
#[cfg(test)]
use crate::proxy_protocol::{proxy_protocol_v1_header, proxy_protocol_v2_header};
...
impl ProxySnapshot {
    ...
    pub fn purge_image_cache(
        ...
        let native_key = crate::cache::image_cache_key(cache_config, &cache_request)
        ...
        if native_primary_key != native_key || cache_key.as_str() != native_key {
            memory_purged |= fluxheim_server::purge_native_memory_cache_primary(
                &vhost.name,
                route_cache.map(|cache| cache.name.as_str()),
                &native_primary_key,
        ...
        if cache_config.range.enabled && cache_config.range.slice.enabled {
            ...
            memory_purged |= native_memory.purged > 0;
            if let Some(storage) = route_cache
                .and_then(|cache| cache.pingora_disk_storage)
                .or(vhost.pingora_disk_storage.filter(|_| route_cache.is_none()))
            {
            ...
            disk_purged |= native_disk.purged > 0;
        }
        Ok(CachePurgeResult {
            vhost: vhost.name.clone(),
            route: route_cache.map(|cache| cache.name.clone()),
    ...
    pub fn purge_indexed_image_cache(
        ...
        let mut disk = route_cache
            ...
            .transpose()?
            .unwrap_or_default();
        let native_disk = fluxheim_server::purge_native_disk_cache_user_tag(
            &vhost.name,
            route_cache.map(|cache| cache.name.as_str()),
            &user_tag,
            request.limit,
        ...
        disk.matched = disk.matched.saturating_add(native_disk.matched);
        disk.purged = disk.purged.saturating_add(native_disk.purged);
        disk.truncated |= native_disk.truncated;
    ...
    pub fn purge_indexed_image_cache_path_prefix(
        ...
        let mut disk = route_cache
            ...
            .transpose()?
            .unwrap_or_default();
        let native_disk = fluxheim_server::purge_native_disk_cache_path_prefix(
            &vhost.name,
            route_cache.map(|cache| cache.name.as_str()),
            &user_tag,
            request.path_prefix,
    ...
    pub fn purge_indexed_image_cache_tag(
        ...
        let mut disk = route_cache
            ...
            .transpose()?
            .unwrap_or_default();
        let native_disk = fluxheim_server::purge_native_disk_cache_tag(
            &vhost.name,
            route_cache.map(|cache| cache.name.as_str()),
            &user_tag,
            request.cache_tag,
    ...
    pub fn purge_stale_image_cache(
        ...
        let mut disk = route_cache
            ...
            .map_err(|error| io::Error::other(error.to_string()))?
            .unwrap_or_default();
        let native_disk = fluxheim_server::purge_native_disk_cache_stale(
            &vhost.name,
            route_cache.map(|cache| cache.name.as_str()),
            &user_tag,
            request.limit,
        ...
        disk.scanned = disk.scanned.saturating_add(native_disk.scanned);
        disk.stale = disk.stale.saturating_add(native_disk.stale);
        disk.purged = disk.purged.saturating_add(native_disk.purged);
        disk.truncated |= native_disk.truncated;
    ...
    pub fn purge_indexed_image_cache_path_pattern(
        ...
        let mut disk = route_cache
            ...
            .transpose()?
            .unwrap_or_default();
        let native_disk = fluxheim_server::purge_native_disk_cache_path_pattern(
            &vhost.name,
            route_cache.map(|cache| cache.name.as_str()),
            &user_tag,
            request.path_pattern,
...
fn apply_upstream_proxy_protocol(
    ...
    let destination = session
        ...
        .copied();
    install_upstream_proxy_protocol(
        peer,
        proxy.upstream_proxy_protocol,
        source,
        destination,
        proxy
            .connect_timeout_secs
...
mod tests {
    ...
    fn access_policy_rejects_malformed_forwarded_for_chain() {
        ...
        let restored = effective_client_ip_from_forwarded_for(
            direct,
            true,
            Some("203.0.113.10, not-an-ip, 10.0.0.253"),
            |ip| matches!(ip, IpAddr::V4(ip) if ip.octets()[0] == 10),
        );
