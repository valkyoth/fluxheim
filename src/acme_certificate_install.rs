...
pub(super) struct UnixFileOwner {
    uid: u32,
    gid: u32,
}

#[cfg(unix)]
pub(super) type ManagedCertificateOwner = Option<UnixFileOwner>;

#[cfg(not(unix))]
pub(super) type ManagedCertificateOwner = ();

#[cfg(unix)]
pub(super) fn managed_certificate_owner(
    storage: &Path,
...
pub(super) fn reject_existing_symlink_in_path(
    path: &Path,
) -> Result<(), AcmeCertificateInstallError> {
