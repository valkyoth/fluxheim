...
mod tests {
    ...
    fn cache_lookup_expectations_validate_object_and_freshness_state() {
        ...
        assert_eq!(
            super::parse_cache_lookup_headers(&[" ETag: \"cached\" ".to_owned()]).unwrap(),
            vec![("etag".to_owned(), "\"cached\"".to_owned())]
        );
        assert!(
            super::parse_cache_lookup_headers(&["Bad Header: value".to_owned()])
                .unwrap_err()
                .to_string()
                .contains("valid HTTP header name")
        );
    ...
    fn cache_key_preview_expectations_validate_policy_layout() {
        ...
        assert!(
            ...
            .unwrap_err()
            .to_string()
            .contains("expected cache predictor enabled, found false")
        );
        assert!(
            super::validate_cache_key_preview_expectations(
                &preview,
