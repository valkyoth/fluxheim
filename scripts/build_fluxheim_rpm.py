...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--features",
        default=DEFAULT_RPM_FEATURES,
        help=(
            "Cargo features to compile into the RPM. Defaults to "
            f"{DEFAULT_RPM_FEATURES}; use 'default' for Cargo's default feature set."
        ),
    )
    parser.add_argument(
        "--list-targets",
...
def validate_features(features: str) -> None:
    if not SAFE_FEATURES.fullmatch(features):
        raise SystemExit(
            "error: --features must be 'default' or a comma-separated list of Cargo feature names"
        )
...
echo "--- Building Fluxheim (${FEATURES}) ---"
RPM_SUFFIX=""
...
and load balancing. This binary RPM was generated by
scripts/build_fluxheim_rpm.py for local installation testing.
...
%dir %attr(0700,root,root) /etc/fluxheim/secrets
