...
TMP_DIR=$(mktemp -d "$SMOKE_TMP_ROOT/fluxheim-storage-bin-smoke.XXXXXX")
...
cleanup() {
    ...
    if [ "$KEEP_LOGS" = "1" ] || [ "$status" -ne 0 ]; then
        echo "storage-bin cache smoke artifacts kept in $TMP_DIR" >&2
    else
        rm -rf "$TMP_DIR"
    fi
}
...
mkdir -p "$TMP_DIR/cache" "$TMP_DIR/run"
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        self.send_header("etag", '"storage-bin-smoke"')
        self.end_headers()
        self.wfile.write(BODY)
...
if [ "$count" != "1" ]; then
    echo "storage-bin cache smoke failed: expected one origin fetch, got $count" >&2
    exit 1
fi
