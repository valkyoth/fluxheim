...
curl -sS -D "$first_headers" -o "$body" -H "Host: cache.test" "http://127.0.0.1:$FLUXHEIM_PORT/asset.png"
...
curl -sS -D "$second_headers" -o "$body" -H "Host: cache.test" "http://127.0.0.1:$FLUXHEIM_PORT/asset.png"
...
conditional_status=$(
    curl -sS -D "$conditional_headers" -o /dev/null -w '%{http_code}' \
        -H "Host: cache.test" \
...
range_status=$(
    curl -sS -D "$range_headers" -o "$range_body" -w '%{http_code}' \
        -H "Host: cache.test" \
        -H "Range: bytes=0-3" \
        "http://127.0.0.1:$FLUXHEIM_PORT/asset.png"
)
