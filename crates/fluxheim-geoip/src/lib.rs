...
mod runtime {
    ...
    fn read_regular_mmdb(path: &Path) -> io::Result<Vec<u8>> {
        ...
        // without openat2(RESOLVE_NO_SYMLINKS), a writable parent directory
        // still has a narrow intermediate-directory substitution window; deploy
        // GeoIP databases under root/service-owned directories.
        ...
        let mut contents =
            Vec::with_capacity(metadata.len().min(MAX_GEOIP_DATABASE_BYTES) as usize);
...
#[cfg(feature = "runtime")]
pub use runtime::{GeoIpPolicyUsage, GeoIpRuntime};
